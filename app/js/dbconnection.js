...
function login(username, password) {
  return new Promise((resolve, reject) => {
    connection.query(
      "SELECT id, username, firstName, lastName FROM users WHERE username = ? AND password = ?",
      [username, password],
      function (err, result) {
        if (err) {
...
function createTicket(username, ticketType) {
  return new Promise((resolve, reject) => {
    ...
    connection.query(
      "SELECT id, unitCost, displayName FROM ticketType WHERE ticketType = ? AND status = ? ORDER BY id Desc LIMIT 1",
      [ticketType, "active"],
      function (err, result) {
        if (err) {
        ...
        if (result.length > 0) {
          connection.query(
            "INSERT INTO tickets (ticketType, description, status, rate, username) VALUES (?,?,?,?,?)",
            [
              ticketType,
            ...
            function (err) {
              if (err) {
                ...
                connection.query(
                  "SELECT id FROM tickets WHERE username = ? ORDER BY id Desc LIMIT 1",
                  [username],
                  function (err, result) {
                    if (err) {
                      reject("Error: " + err.message);
                        ...
                        connection.query(
                          ...
                          function (err) {
                            if (err) {
                              ...
                              connection.query(
                                "SELECT ticketNumber, createdDate, description FROM tickets WHERE username = ? ORDER BY id Desc LIMIT 1",
                                [username],
                                function (err, result) {
                                  if (err) {
                                    reject("Error: " + err.message);
  ...
  return new Promise((resolve, reject) => {
    ...
    connection.query(
      ...
      function (err, result) {
        ...
        if (result.length > 0) {
          connection.query(
            "INSERT INTO specialTickets (ticketType, vehicleRegistration, description, status, startDate, endDate, rate, username) VALUES (?,?,?,?,?,?,?)",
            [
              ticketType,
              vehicleRegistration,
              result[0].displayName,
            ...
            function (err) {
              if (err) {
                ...
                // Return last created ticket number to main program
                connection.query(
                  "SELECT vehicleRegistration, createdDate, description FROM specialTickets WHERE username = ? ORDER BY id Desc LIMIT 1",
                  [username],
                  function (err, result) {
                    if (err) {
                      reject("Error: " + err.message);
...
function getTicket(ticketNumber, username) {
  return new Promise((resolve, reject) => {
    // Select last created ticket for the current user
    connection.query(
      "SELECT * FROM tickets WHERE ticketNumber = ? AND username = ? ORDER BY id Desc LIMIT 1",
      [ticketNumber, username],
      function (err, result) {
        if (err) {
          reject("Error: " + err.message);
        } else {
...
function createTicketType(ticketType, unitCost, username) {
  return new Promise((resolve, reject) => {
    ...
    connection.query(
      "SELECT id FROM ticketType WHERE ticketType = ? AND status = ? ORDER BY id Desc LIMIT 1",
      [ticketType, "open"],
      function (err, result) {
        if (err) {
          ...
          connection.query(
            "UPDATE ticketType SET status = ? WHERE id = ?",
            ["closed", result[0].id],
            function (err) {
              if (err) {
                reject("Error: " + err.message);
        ...
        connection.query(
          "INSERT INTO ticketType (ticketType, unitCost, status, username) VALUES (?,?,?,?)",
          [ticketType, unitCost, "open", username],
          function (err) {
            if (err) {
...
function retrieveTicketTypeInfo(ticketType) {
  return new Promise((resolve, reject) => {
    connection.query(
      "SELECT ticketType, unitCost, status, createdDate, displayName, description, username FROM ticketType WHERE ticketType = ?",
      [ticketType],
      function (err, result) {
        if (err) {
...
function createReceipt(
  ...
  return new Promise((resolve, reject) => {
    // Checks to see if ticket exists in database
    connection.query(
      "SELECT ticketNumber, ticketType, createdDate, closedDate, status, rate, ticketCost, balance, username FROM tickets WHERE ticketNumber = ? ORDER BY id Desc LIMIT 1",
      [ticketNumber],
      function (err, result) {
        if (err) {
          ...
          if (result) {
            ...
            connection.query(
              "UPDATE tickets SET status = ?, ticketCost = ?, balance = ? WHERE ticketNumber = ?",
              [ticketStatus, ticketCost, balance, ticketNumber],
              function (err) {
                if (err) {
                  reject("Cannot create receipt - Error: " + err.message);
                } else {
                  connection.query(
                    ...
                    function (err) {
                      if (err) {
                        ...
                        connection.query(
                          "SELECT * FROM receipts WHERE id = LAST_INSERT_ID()",
                          function (err, result) {
                            if (err) {
                            } else {
                              resolve(result[0]);
...
function createLostTicketReceipt(
  ticketType,
  receiptStatus,
  ticketCost,
  amountPaid,
  ...
  return new Promise((resolve, reject) => {
    connection.query(
      "INSERT INTO receipts (ticketType, status, ticketCost, amountPaid, balance, amountDue, paymentMethod, chequeNumber, username) VALUES (?,?,?,?,?,?,?,?,?)",
      [
        ticketType,
        receiptStatus,
        ticketCost,
...
function getReceipt(ticketNumber, username) {
  return new Promise((resolve, reject) => {
    ...
    connection.query(
      "SELECT * FROM receipts WHERE ticketNumber = ? AND username ORDER BY id Desc LIMIT 1",
      [ticketNumber, username],
      function (err, result) {
        if (err) {
          reject("Error: " + err.message);
...
function getLastReceipt(username) {
  return new Promise((resolve, reject) => {
    ...
    connection.query(
      "SELECT * FROM receipts WHERE username = ? ORDER BY id Desc LIMIT 1",
      [username],
      function (err, result) {
        if (err) {
          reject("Error: " + err.message);
